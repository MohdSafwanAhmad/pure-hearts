/**
 * @file route.ts
 * @description API Route: /api/campaigns/search
 * Handles search requests for campaigns/projects by delegating logic
 * to the reusable server-side function `searchCampaigns` located in src/api/campaigns.ts.
 *
 * This route simply:
 *  1. Extracts query params (q, sort, category)
 *  2. Calls searchCampaigns() with those params
...
import { searchCampaigns } from "@/src/api/campaigns"; // Reusable search logic

/**
 * @function GET
 * @description Route handler for GET /api/campaigns/search
 * @param {NextRequest} req - Incoming request object
 * @returns {NextResponse} JSON response containing search results
 */
export async function GET(req: NextRequest) {
  try {
    // === Extract URL params ===
    const { searchParams } = new URL(req.url);
    const q = searchParams.get("q")?.trim() || ""; // Search query text
    ...
    // === Call server-side reusable search function ===
    const result = await searchCampaigns({ q, sort, category });
