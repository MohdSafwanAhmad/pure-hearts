...
export async function addProject(
  ...
  // 2) Organization check (need slug for revalidation)
  ...
  // 6) Revalidate pages that show the new project card
  revalidatePath("/campaigns");
  if (organization.slug) {
    revalidatePath(`/organizations/${organization.slug}`);
    // If you also have a nested list route, add it here:
    // revalidatePath(`/organizations/${organization.slug}/projects`);
  }
